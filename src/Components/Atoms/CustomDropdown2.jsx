...
const CustomDropdown2 = ({ data, dataHandler }) => {
  ...
  return (
    <>
      <div className=" relative">
        <div
          className="flex justify-end items-center px-3 bg-[#F7F8FA]  rounded-[2px] "
          onClick={() => {
            setSelect(!select);
          }}
        >
          <span className="flex grow text-[#1875F0] text-[13px] mr-2">
            {selectValue}
          </span>
          <svg
            width="7"
        ...
        <div
          className={`bg-white absolute z-10 left-0 right-0  transition-all duration-150 overflow-hidden  shadow-md ${
            select
              ? "h-[99px] border-lghtgry border border-t-transparent rounded-b-[6px]"
              : "border-transparent h-0"
          }`}
        >
          {data.map((datum, index) => (
            <p
              className={`text-[#414042] h-[33px] flex items-center pl-3 text-[13px]  transition-all duration-100 hover:bg-[#f4f4f4] ${
